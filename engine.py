...
def train_one_epoch(cfg, model, optimizer, data_loader, device, epoch, tfboard=None):
    ...
    for i, (images, targets) in enumerate(
        ...
        # Replace NaN losses with zero
        for k in loss_dict_reduced:
            if torch.isnan(loss_dict_reduced[k]):
                print(f"{k} is NaN, setting it to zero")
                loss_dict_reduced[k] = torch.tensor(0.0, device=loss_dict_reduced[k].device)
